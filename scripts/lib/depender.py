...
class Converter():
    ...
    def _fix_aux_tag_rel(self):
        ...
        for address, node in self.dg.nodes.items():
            ...
            if node['rel'] in {'aux', 'dep'} and self.dg.get_by_address(node['head'])['rel'] == 'aux':
                self.dg.get_by_address(address).update({'head': self.dg.get_by_address(node['head'])['head']})
        ...
        try:
            for address, node in self.dg.nodes.items():
                if node['ctag'] == 'PUNCT':
                    ...
                    elif address+1 in self.dg.nodes \
                    and self.dg.get_by_address(address+1)['rel'] == 'conj':
                        ...
                        if address-1 in self.dg.nodes and self.dg.get_by_address(address-1)['head'] == address:
                            ...
                            self.dg.get_by_address(address).update({'head': address-1})
                        elif address-1 in self.dg.nodes and self.dg.get_by_address(address-1)['rel'] == 'conj' and node['head'] > self.dg.get_by_address(address-1)['head']:
                            self.dg.get_by_address(address).update({'head': address-1})
                        elif address-2 in self.dg.nodes and address-1 in self.dg.nodes and node['head'] == address-1:
                            if self.dg.get_by_address(address-1)['head'] == address:
                                self.dg.get_by_address(address-1).update({'head': address-2})
                        elif address-2 in self.dg.nodes and address-1 in self.dg.nodes and self.dg.get_by_address(address-2)['rel'] == 'advcl' and node['head'] > self.dg.get_by_address(address-2)['head'] and self.dg.get_by_address(address-1)['head'] == address-2:
                            self.dg.get_by_address(address).update({'head': address-1})
                        elif address+2 in self.dg.nodes and address-1 in self.dg.nodes and self.dg.get_by_address(address+2)['head'] == address-1 and self.dg.get_by_address(address-1)['rel'] == 'amod' and self.dg.get_by_address(address-1)['head'] <= node['head']:
                            self.dg.get_by_address(address).update({'head': address-1})
                        elif address+2 in self.dg.nodes and address-1 in self.dg.nodes and self.dg.get_by_address(address+2)['head'] != '_' and self.dg.get_by_address(address-1)['rel'] == 'amod' and self.dg.get_by_address(address-1)['head'] <= node['head']:
                            self.dg.get_by_address(address).update({'head': address-1})
                        elif address+2 in self.dg.nodes and address-1 in self.dg.nodes and self.dg.get_by_address(address+2)['head'] == address-1 and node['head'] < address-1:
                            self.dg.get_by_address(address).update({'head': address-1})
                        elif address+1 in self.dg.nodes and address-1 in self.dg.nodes and node['head'] > address and self.dg.get_by_address(address+1)['head'] < address+1:
                            self.dg.get_by_address(address).update({'head': address-1})
        ...
        try:
            for address, node in self.dg.nodes.items():
                ...
                if node['rel'] == 'conj' and node['head'] == address+1: 
                    if self.dg.get_by_address(address+1)['rel'] == 'obl': # TODO: bæta við 'obl:arg'?
                        if node['ctag'] == 'NOUN':
                        ...
                        if node['ctag'] == 'ADJ':
                            self.dg.get_by_address(address).update({'rel': 'amod'})
                    elif self.dg.get_by_address(address+1)['rel'] in {'amod', 'acl:relcl'}:
                        if node['ctag'] == 'NOUN':
                ...
                # If a node with the deprel 'conj' is dependent on the node after the following node
                    ...
                    elif self.dg.get_by_address(address+2)['rel'] == 'obj' and self.dg.get_by_address(address-2)['rel'] == 'amod' and self.dg.get_by_address(address-2)['head'] == node['head'] and self.dg.get_by_address(address-1)['rel'] == 'cc':
                        if node['ctag'] == 'ADJ':
                            self.dg.get_by_address(address).update({'head': address-2})
                    ...
                    if self.dg.get_by_address(address+3)['rel'] == 'obl':
                        ...
                        if node['ctag'] == 'NOUN' and self.dg.get_by_address(address+1)['rel'] == 'cop' and self.dg.get_by_address(address+1)['head'] == node['head']:
                            self.dg.get_by_address(address).update({'rel': 'nsubj'})
                    elif self.dg.get_by_address(address+3)['rel'] == 'acl:relcl':
                        if self.dg.get_by_address(address-2)['rel'] == 'advmod' and self.dg.get_by_address(address-2)['head'] == node['head']:
                            self.dg.get_by_address(address).update({'head': address-2})
                    elif self.dg.get_by_address(address+3)['rel'] == 'nmod:poss' and self.dg.get_by_address(address-4)['rel'] == 'amod' and self.dg.get_by_address(address-4)['head'] == node['head']:
                        self.dg.get_by_address(address).update({'head': address-4})
                    ...
                    if self.dg.get_by_address(address+7)['rel'] == 'ccomp' and self.dg.get_by_address(address-3)['rel'] == 'nsubj' and self.dg.get_by_address(address-3)['head'] == node['head']:
                        self.dg.get_by_address(address).update({'head': address-3})
                    elif self.dg.get_by_address(address-7)['rel'] == 'obl' and self.dg.get_by_address(address-7)['head'] == node['head']:
                        self.dg.get_by_address(address).update({'head': address-7})
                    ...
                    if self.dg.get_by_address(address-4)['rel'] == 'dislocated' and self.dg.get_by_address(address-4)['head'] == node['head']:
                        self.dg.get_by_address(address).update({'head': address-4})
    ...
    def _fix_aclrelcl_rel(self):
        ...
        for address, node in self.dg.nodes.items():
            if node['rel'] == 'acl:relcl' and node['head'] == address+2 and self.dg.get_by_address(address+2)['head'] == address:
            ...
            for address, node in self.dg.nodes.items():
                    if node['address'] == node['head']:
                        if address != 1:
                            ...
                            elif address-12 in self.dg.nodes and self.dg.get_by_address(address-12)['ctag'] == 'VERB':
                                self.dg.get_by_address(address).update({'head': address-12})
                            elif address-4 in self.dg.nodes and self.dg.get_by_address(address-4)['ctag'] == 'VERB':
                                self.dg.get_by_address(address).update({'head': address-4})
                            elif address-3 in self.dg.nodes and self.dg.get_by_address(address-3)['ctag'] == 'VERB':
                                self.dg.get_by_address(address).update({'head': address-3})
                            elif address-2 in self.dg.nodes and self.dg.get_by_address(address-2)['rel'] == 'root':
                                self.dg.get_by_address(address).update({'head': address-2})
                            elif address-2 in self.dg.nodes and self.dg.get_by_address(address-2)['ctag'] == 'VERB':
                                self.dg.get_by_address(address).update({'head': address-2})
                            elif address-1 in self.dg.nodes and self.dg.get_by_address(address-1)['ctag'] == 'VERB':
                                self.dg.get_by_address(address).update({'head': address-1})
                            elif address-4 in self.dg.nodes and self.dg.get_by_address(address-4)['rel'] == 'conj' and node['rel'] == 'acl:relcl':
                                self.dg.get_by_address(address).update({'head': address-4})
                        else:
                            if address+1 in self.dg.nodes.items() and self.dg.get_by_address(address+1)['ctag'] == 'VERB':
                                self.dg.get_by_address(address).update({'head': address+1})
                            elif address+2 in self.dg.nodes.items() and self.dg.get_by_address(address+2)['word'] != None and self.dg.get_by_address(address+2)['ctag'] == 'VERB':
                                self.dg.get_by_address(address).update({'head': address+2})
                            elif address+3 in self.dg.nodes.items() and self.dg.get_by_address(address+3)['word'] != None and self.dg.get_by_address(address+3)['ctag'] == 'VERB':
                                self.dg.get_by_address(address).update({'head': address+3})
                            elif address+4 in self.dg.nodes.items() and self.dg.get_by_address(address+4)['word'] != None and self.dg.get_by_address(address+4)['ctag'] == 'VERB':
                                self.dg.get_by_address(address).update({'head': address+4})
        except RuntimeError:
            pass
    ...
    def _fix_flat_foreign(self):
        ...
        try:
            for address, node in self.dg.nodes.items():
                if node['ctag'] == 'X': 
                    if address+1 in self.dg.nodes and self.dg.get_by_address(address+1)['head'] == address and self.dg.get_by_address(address+1)['ctag'] == 'PROPN':
                    ...
                    if address-1 in self.dg.nodes and self.dg.get_by_address(address-1)['head'] == address and self.dg.get_by_address(address-1)['ctag'] == 'ADP':
